...
class AuditPurchaseService:
    """Consulta y formatea el historial de auditoría de compras.

    Comparte la lógica entre la vista /auditoria/compras y las descargas
    PDF/Excel para que ambas muestren exactamente los mismos datos.
    """

    @staticmethod
    def list_purchase_audits(current_user, filtros=None):
        filtros = filtros or {}
        ...
        # 3. Filtro por sede (optativo): compras cuyo creador pertenece a esa sede
        if filtros.get('sede'):
            query = query.filter(
                PurchaseCreator.locations.any(Location.id == filtros['sede'])
            )
        # 4. Filtrar si el rol es Finanzas (solo ve sus sedes asignadas)
        elif user_role_name in ['finance', 'finanzas']:
        ...
        # 5. Filtro por rango de fechas (optativo)
        if filtros.get('desde'):
            query = query.filter(PurchaseAuditLog.timestamp >= filtros['desde'])
        if filtros.get('hasta'):
            query = query.filter(PurchaseAuditLog.timestamp <= filtros['hasta'])

        # 6. Consulta ordenada por fecha descendente
        results = query.order_by(PurchaseAuditLog.timestamp.desc()).all()

        return AuditPurchaseService._formatear(current_user, results)
